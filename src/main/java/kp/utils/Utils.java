...
public class Utils {
    ...
    public static Consumer<Information> createConsumer(PulsarClient pulsarClient, String topic,
                                                       MessageListener<Information> messageListener)
            throws PulsarClientException {
    ...
    /**
     * Creates the {@link Consumer} and subscribes to it with the <i>Failover</i> {@link SubscriptionType}.
     *
     * @param pulsarClient the {@link PulsarClient}
     * @param topic        the topic
     * @return the {@link Consumer}
    ...
    private static List<Employee> createEmployees(long departmentId) {
        ...
        return IntStream.rangeClosed(1, 3).mapToObj(i -> {
            long id = 10 * departmentId + i;
            return new Employee(id, "f-name-" + id, "l-name-" + id, Title.values()[i - 1]);
        }).toList();
    }
    ...
    public static String createLabel() {
        return "%03d".formatted(random.nextInt(999));
    }
